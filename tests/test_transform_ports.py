...
def test_port_entity_table_keys_match_backend_extracts() -> None:
...
def test_ports_to_entities_warns_on_duplicate_first_row_join(stub_sdk, caplog) -> None:
...
def test_ports_to_entities_maps_config_state_and_vlans_onto_one_interface(stub_sdk) -> None:
    entities = transform.ports_to_entities(_tables(), record=_port_record())
...
def test_ports_to_entities_config_only_port_still_syncs_admin_state(stub_sdk) -> None:
    """No port-state: admin state syncs; type omitted so degrade cannot invent other."""
    entities = transform.ports_to_entities(_tables(port_states=[], vlan_properties=[]), record=_port_record())
...
def test_ports_to_entities_state_only_port_still_syncs_link_state(stub_sdk) -> None:
...
def test_ports_to_entities_admin_down_and_link_down_are_independent(stub_sdk) -> None:
...
def test_ports_to_entities_omits_type_for_unverified_enum_codes(stub_sdk) -> None:
    ...
    entities = transform.ports_to_entities(
        _tables(port_states=[state], vlan_properties=[]),
        record=_port_record(),
    )
...
def test_ports_to_entities_fiber_gig_port_maps_to_sfp_type(stub_sdk) -> None:
...
def test_ports_to_entities_omits_fields_for_off_spec_string_codes(stub_sdk) -> None:
    ...
    port = (
        transform.ports_to_entities(_tables(port_states=[state], vlan_properties=[]), record=_port_record())[
            0
        ]
        ._kw["interface"]
        ._kw
...
def test_ports_to_entities_capabilities_scoped_per_device(stub_sdk, caplog) -> None:
...
def test_ports_to_entities_warns_on_per_device_capability_duplicates(stub_sdk, caplog) -> None:
    caps = [
...
def test_ports_to_entities_maps_poe_mode_pse_when_supported(stub_sdk) -> None:
    poe_state = {
...
def test_ports_to_entities_omits_poe_when_not_supported(stub_sdk) -> None:
    poe_state = {
        "device_id": "cs-uuid-42",
        "asset_interface_id": "if-uuid-1",
        "supported": False,
...
def test_ports_to_entities_ignores_poe_config_enable_without_supported(stub_sdk) -> None:
...
def test_ports_to_entities_maps_poe_classification_bt_and_omits_unmapped(stub_sdk) -> None:
...
def test_ports_to_entities_maps_oper_duplex_half(stub_sdk) -> None:
...
def test_ports_to_entities_falls_back_to_config_duplex_auto(stub_sdk) -> None:
...
def test_ports_to_entities_does_not_fallback_for_non_unset_oper_duplex(stub_sdk) -> None:
...
def test_ports_to_entities_prefers_oper_duplex_over_config(stub_sdk) -> None:
...
def test_ports_to_entities_does_not_use_native_vlan_without_vlan_properties(stub_sdk) -> None:
...
def test_ports_to_entities_vlan_properties_still_apply_with_native_vlan_present(stub_sdk) -> None:
    config = {**PORT_CONFIG, "native_vlan": 99, "port_mode": True}
    entities = transform.ports_to_entities(_tables(port_configs=[config]), record=_port_record())
...
def test_ports_to_entities_rewrites_colon_ports_to_native_notation(stub_sdk) -> None:
    ...
    entities = transform.ports_to_entities(
        ...
        record=_port_record(function="Fabric Engine"),
    )
...
def test_ports_to_entities_keeps_colon_ports_for_switch_engine(stub_sdk) -> None:
    ...
    entities = transform.ports_to_entities(
        ...
        record=_port_record(function="Switch Engine"),
    )

    assert entities[0]._kw["interface"]._kw["name"] == "1:52"


def test_ports_to_entities_emits_interface_ip_addresses(stub_sdk) -> None:
    ips = [
        {
            "asset_interface_id": "if-uuid-1",
            "address": "10.0.0.2",
...
def test_ports_to_entities_ip_stub_omits_type_when_port_state_missing(stub_sdk) -> None:
...
def test_ports_to_entities_emits_svi_ips_via_vlan_interface_name(stub_sdk) -> None:
...
def test_ports_to_entities_skips_orphan_ips_without_known_interface_name(stub_sdk) -> None:
...
def test_ports_to_entities_skips_interface_ips_without_mask_length(stub_sdk) -> None:
    ips = [
        {
            "asset_interface_id": "if-uuid-1",
            "address": "10.0.0.2",
...
def test_ports_to_entities_untagged_only_is_access_mode(stub_sdk) -> None:
    vlan = {**VLAN_PROPERTIES, "vlans": [{"vlan_number": 10}]}
    entities = transform.ports_to_entities(_tables(vlan_properties=[vlan]), record=_port_record())
...
def test_ports_to_entities_omits_mode_when_no_vlan_rows(stub_sdk) -> None:
    """FLEX-UNI/Fabric-Attach ports can be mapped to an I-SID instead of a
    VLAN -- inventing an access mode would misrepresent them.
    """
    port = (
        transform.ports_to_entities(_tables(vlan_properties=[]), record=_port_record())[0]
        ._kw["interface"]
        ._kw
    )
    assert "mode" not in port
...
def test_ports_to_entities_omits_reserved_untagged_vlan(stub_sdk) -> None:
...
def test_ports_to_entities_strips_reserved_tagged_vids(stub_sdk) -> None:
...
def test_ports_to_entities_omits_vlan_and_mode_when_only_reserved(stub_sdk) -> None:
    ...
    port = (
        transform.ports_to_entities(_tables(vlan_properties=[vlan]), record=_port_record())[0]
        ._kw["interface"]
        ._kw
    )
    assert "mode" not in port
...
def test_ports_to_entities_ports_join_on_interface_id_not_row_order(stub_sdk) -> None:
...
def test_ports_to_entities_nests_device_site_role_and_type(stub_sdk) -> None:
    ...
    assert device["device_type"]._kw["model"] == "FabricEngine-5320-48P-8XE"
...
def test_ports_to_entities_warns_on_conflicting_port_vlan(stub_sdk, caplog) -> None:
    ...
    port = (
        transform.ports_to_entities(_tables(vlan_properties=[vlan_a, vlan_b]), record=_port_record())[0]
        ._kw["interface"]
        ._kw
    )
